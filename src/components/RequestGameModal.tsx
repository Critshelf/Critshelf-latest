...
import { X, Calendar, Megaphone, Loader2, Check, Clock, MapPin } from 'lucide-react';
...
const RequestGameModal: React.FC<RequestGameModalProps> = ({ isOpen, onClose, groupId, game }) => {
  const { user } = useUser();
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      for (const eventId of selectedEventIds) {
        ...
        batch.push(addDoc(collection(db, 'groupRequests'), {
          groupId,
          eventId,
          ...
          >
            <div className="bg-white/5 p-8 text-white relative overflow-hidden border-b border-white/10">
              ...
              <div className="relative flex items-center gap-4">
                ...
                <div>
                  <h2 className="text-2xl font-black tracking-tight underline decoration-gold-accent/30 decoration-4 underline-offset-4">Request Game</h2>
                  <p className="text-white/40 text-[10px] font-black uppercase tracking-widest mt-1">Ask the owner to bring it!</p>
                </div>
              </div>
            </div>
            ...
            <div className="p-8 space-y-6 max-h-[60vh] overflow-y-auto no-scrollbar">
              ...
              <div className="space-y-4">
                <h4 className="text-xs font-black text-white/20 uppercase tracking-widest ml-2">Select Upcoming Events</h4>
                ...
                {loading ? (
                  ...
                  <div className="text-center py-10 bg-white/5 rounded-3xl border border-dashed border-white/10">
                    <p className="text-white/20 font-bold">No upcoming events scheduled.</p>
                  </div>
                ) : (
                    ...
                    {events.map(event => (
                      ...
                      >
                        <div className={cn(
                          ...
                          {selectedEventIds.includes(event.id) && <Check className="w-4 h-4" />}
                        </div>
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-3 mt-1">
                            <span className="text-[10px] font-black text-white/40 flex items-center gap-1 uppercase tracking-widest">
                              <Calendar className="w-3 h-3" /> {formatDate(event.dateTime)}
                            </span>
              ...
              >
                {isSubmitting ? (
                  ...
                  <>
                    ...
                    Send {selectedEventIds.length} Request{selectedEventIds.length !== 1 && 's'}
                  </>
                )}
              </button>
            </div>
