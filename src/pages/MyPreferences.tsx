...
export default function MyPreferences() {
  const { user, profile, refreshProfile, updateNotificationPreferences } =
    useUser();
  ...
  useEffect(() => {
    if (profile) {
      if (profile.notificationPreferences) {
        setPrefs({
          moderation: profile.notificationPreferences.moderation ?? true,
          social: profile.notificationPreferences.social ?? true,
          library: profile.notificationPreferences.library ?? true,
          groups: profile.notificationPreferences.groups ?? true,
        });
      }
      setLoading(false);
    }
  ...
  const handleToggle = async (key: keyof typeof prefs) => {
    ...
    try {
      await updateDoc(doc(db, "users", user.uid), {
        [`notificationPreferences.${String(key)}`]: newValue,
      });
    } catch (error) {
      // Revert if error
      setPrefs((prev) => ({ ...prev, [key]: !newValue }));
